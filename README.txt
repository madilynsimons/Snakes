Snakes

*** IN REGARDS TO AttackLibrary.txt ***
AttackLibrary.txt holds a list of every possible Attack
in the game.  The parameters exist as follows:

id    -- some non-zero integer used to identify the Attack
name  -- what the Attack is called
maxPP -- some non-zero integer representing the max PP of
         an attack, or, how many times that attack can be
         used
size  -- the size of the attack, or how many slots the attack
         takes up.  this can be 0, 1, 2, 3, 4, or 6
         NOTE: 0 is null

Please follow the conventions or else everything will explode.